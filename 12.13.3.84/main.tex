...
P_X(X = k)= \comb{n}{k}(1-p)^{n-k}p^{k}
\end{equation}
where,$P_X(X = k)$ is the probability of getting $k$ number of queens in trails 
\begin{table}[h]
\centering
...
\end{tabular}
\end{table}
The probability for the trails to succeed to 2 times is $P_X(X = 2)$.
Using \eqref{eq:ncert/12/13/3/84/} we get,
\begin{equation}
\begin{split}
P_X(X = 2)&= \comb{2}{2}(1-\frac{1}{13})^{2-2}(\frac{1}{13})^{2}\\
